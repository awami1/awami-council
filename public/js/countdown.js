// Countdown — يقرأ من بيانات PHP المُمرَّرة مباشرة (بدون API call)
// إذا لم تُمرَّر بيانات يفزع للـ API كاحتياطي
async function initCountdown(preloaded) {
let tDate = null;
let visible = true;

if (preloaded !== undefined) {
  // بيانات من PHP SSR
  if (preloaded && preloaded.date) {
    tDate   = new Date(preloaded.date);
    visible = preloaded.visible !== false;
  }
} else {
  // احتياطي: اقرأ من API
  try {
