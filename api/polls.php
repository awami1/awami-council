...
// ──────────────────────────────────────────────────────────────
// HELPERS (polls-specific, supplements config.php)
// ──────────────────────────────────────────────────────────────
    ...
    if (isSQLite()) {
        $pdo->prepare(
            'INSERT INTO poll_votes (poll_id, option_id, user_id, voted_at)
             VALUES (:poll_id, :option_id, :user_id, CURRENT_TIMESTAMP)
             ON CONFLICT(poll_id, user_id) DO UPDATE SET option_id = :option_id2, voted_at = CURRENT_TIMESTAMP'
        )->execute([
            ':poll_id'    => $pollId,
            ':option_id'  => $option['id'],
            ':user_id'    => $userId,
            ':option_id2' => $option['id'],
        ]);
    } else {
        $pdo->prepare(
